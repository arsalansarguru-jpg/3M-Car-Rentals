...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [userDisplayName, setUserDisplayName] = useState("Arsalan Sarguru");
  const [userInitials, setUserInitials] = useState("AS");
  const [userRole, setUserRole] = useState("Chief Operating Officer");
  ...
  useEffect(() => {
    setIsAdmin(pathname?.startsWith("/dashboard/admin") ?? false);
  }, [pathname]);
    ...
    async function fetchUser() {
      ...
      if (user) {
        ...
        if (profile) {
          ...
          const fullName = [fname, lname].filter(Boolean).join(" ") || user.email?.split("@")[0] || "Admin";
          const initials = [fname[0], lname[0]].filter(Boolean).join("").toUpperCase() || "A";
          const roleObj = Array.isArray(profile.role) ? profile.role[0] : profile.role;
          const roleName = (roleObj as any)?.name || "Administrator";
          setUserDisplayName(fullName);
          setUserInitials(initials);
          setUserRole(roleName.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase()));
        }
      }
    }
    fetchUser();
  ...
  // Variant Mapping configuration for side navigation items
  const adminNav = [
    { name: "Overview", href: "/dashboard/admin", icon: LayoutDashboard, color: "blue", activeText: "text-blue-400" },
    { name: "Smart Calendar", href: "/dashboard/admin/bookings", icon: CalendarIcon, color: "blue", activeText: "text-blue-400" },
    { name: "Customer 360", href: "/dashboard/admin/customers", icon: Users, color: "pink", activeText: "text-pink-400" },
    { name: "Dynamic Pricing", href: "/dashboard/admin/pricing", icon: TrendingUp, color: "cyan", activeText: "text-cyan-400" },
    { name: "Revenue Intel", href: "/dashboard/admin/revenue", icon: Wallet, color: "purple", activeText: "text-purple-400" },
    { name: "Fleet Health", href: "/dashboard/admin/fleet-health", icon: Car, color: "cyan", activeText: "text-cyan-400" },
    { name: "Staff Ops", href: "/dashboard/admin/staff-performance", icon: Activity, color: "indigo", activeText: "text-indigo-400" },
    { name: "Licensing", href: "/dashboard/admin#licensing", icon: ShieldCheck, color: "slate", activeText: "text-slate-400" },
    
    // Fleet Management Sub-pages
    { name: "Fleet Overview", href: "/dashboard/admin/fleet-management/overview", icon: LayoutDashboard, group: "Fleet Management", color: "cyan", activeText: "text-cyan-400" },
    { name: "Vehicle Inventory", href: "/dashboard/admin/fleet-management/inventory", icon: Car, group: "Fleet Management", color: "cyan", activeText: "text-cyan-400" },
    { name: "Vehicle Gallery", href: "/dashboard/admin/fleet-management/gallery", icon: Image, group: "Fleet Management", color: "cyan", activeText: "text-cyan-400" },
    { name: "Availability Calendar", href: "/dashboard/admin/fleet-management/availability", icon: CalendarIcon, group: "Fleet Management", color: "cyan", activeText: "text-cyan-400" },
    { name: "Maintenance", href: "/dashboard/admin/fleet-management/maintenance", icon: Activity, group: "Fleet Management", color: "cyan", activeText: "text-cyan-400" },
    { name: "Documents", href: "/dashboard/admin/fleet-management/documents", icon: FileText, group: "Fleet Management", color: "cyan", activeText: "text-cyan-400" },
    { name: "Pricing", href: "/dashboard/admin/fleet-management/pricing", icon: TrendingUp, group: "Fleet Management", color: "cyan", activeText: "text-cyan-400" },
    { name: "Vehicle Analytics", href: "/dashboard/admin/fleet-management/analytics", icon: Activity, group: "Fleet Management", color: "cyan", activeText: "text-cyan-400" },
  ];

  const clientNav = [
    { name: "My Bookings", href: "/dashboard/client", icon: Car, color: "blue", activeText: "text-blue-400" },
    { name: "Support", href: "/contact", icon: MessageSquare, color: "pink", activeText: "text-pink-400" },
  ];

  const navigationItems = isAdmin ? adminNav : clientNav;

  // Render specific active indicator shadows
    ...
    <div className="min-h-screen bg-[#121210] flex text-gray-200 overflow-hidden font-sans">
      ...
      >
        ...
        <div className="flex-1 py-6 flex flex-col gap-1.5 px-3 overflow-y-auto custom-scrollbar overflow-x-hidden">
          {navigationItems.filter((item: any) => !item.group).map((item) => {
            const isActive = pathname === item.href;
            return (
              <Link
                key={item.name}
                href={item.href}
                className="group relative flex items-center rounded-[20px] transition-all duration-200"
              >
                {/* Active Indicator Line */}
                {isActive && (
                  <motion.div 
                    layoutId="activeNav" 
                    className={`absolute left-0 top-1 bottom-1 w-1 rounded-full ${getGlowLine(item.color)}`} 
                  />
                )}
                
                <div className={`flex items-center w-full px-4 py-3 rounded-[20px] transition-all border border-transparent duration-200 ${
                  isActive 
                    ? `border-white/10 ${getGlowStyles(item.color)}` 
          ...
          {/* Grouped section: Fleet Management */}
          {isAdmin && (
            <div className="mt-6 pt-4 border-t border-white/5 space-y-1">
              ...
              {navigationItems.filter((item: any) => item.group === "Fleet Management").map((item) => {
                const isActive = pathname === item.href;
                return (
                  <Link
                    key={item.name}
                  ...
                  >
                    ...
                    {isActive && (
                      <motion.div 
                        layoutId="activeNavFleet" 
                        className={`absolute left-0 top-1 bottom-1 w-0.5 rounded-full ${getGlowLine(item.color)}`} 
                      />
                    )}
                ...
                {navigationItems.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.name}
                      href={item.href}
                      className={`flex items-center w-full px-4 py-3 rounded-[20px] transition-all border border-transparent duration-200 ${
                        isActive 
                          ? `border-white/10 ${getGlowStyles(item.color)}` 
                          : "text-white/55 hover:text-white hover:bg-white/[0.04]"
                      }`}
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="hidden lg:flex items-center gap-2">
                ...
                <span style={{ fontFamily: "var(--font-body)", fontSize: "0.875rem", fontWeight: 600, color: "#ffffff" }}>
                  {pathname === "/dashboard/admin" ? "Executive Overview" :
                   pathname === "/dashboard/admin/bookings" ? "Smart Calendar" :
                   pathname?.includes("/dashboard/admin/customers") ? "Customer 360" :
                   pathname === "/dashboard/admin/pricing" ? "Dynamic Pricing" :
                   pathname === "/dashboard/admin/revenue" ? "Revenue Intel" :
                   pathname === "/dashboard/admin/fleet-health" ? "Fleet Health" :
                   pathname === "/dashboard/admin/staff-performance" ? "Staff Ops" :
                   pathname === "/dashboard/client" ? "My Bookings" :
                   "Operations"}
                </span>
             </div>
          </div>
